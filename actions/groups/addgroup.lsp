	...
	[0x0](LLL
		{
									...
									[0x0](LLL
										{
											...
											(when (= (calldataload 0) "init") 
												{
													...
													(unless (&& (= (CALLER) @0x0) (> (calldataload 64) 0x40) (> (calldataload 32) 0x40) )
														{
															[0x0] 0
															(return 0x0 32)
														}
													...
													[0x0] "get"
													[0x20] "users"
													(call (- (GAS) 100) @@0x10 0 0x0 64 0x60 32)
													
													[0x20] "getnickaddr"
													[0x40] (calldataload 32)
													(call (- (GAS) 100) @0x60 0 0x20 64 0x0 32)
													
													; Stop if a user with the given name already exists.
													(when @0x0
														{
															[0x0] 0
															(return 0x0 32)
													...
													; Stop if no such group type exists.
													...
													[[0x11]] (calldataload 32) ; group name
													[[0x12]] (calldataload 64) ; group type
											...
											(when (= (calldataload 0) "execute")
												{													
													...
													[0x40] "create"
													[0x60] @@0x12
													(call (- (GAS) 100) @0x0 0 0x40 64 0x0 32) ; Create a new group of the given type.
													
													; If we didn't get a proper group contract address, stop. This should not happen.
													(unless @0x0 (return 0x0 32) )
													
													; Register the group as a user with name @@0x11.
													[0x40] "reg"
													[0x60] @@0x11
													[0x80] @0x0
													(call (- (GAS) 100) @0x20 0 0x40 96 0x20 32)
													
													(unless @0x20 (return 0x20 32) ) ; If registration failed - cancel.
													
													; Finally, set the name of the group.
													[0x40] "setname"
													[0x60] @@0x11
			...
			; RETURNS: 1 if successful, 0 if not.
			; NOTES: Auto executes the action.
			; INTERFACE Action

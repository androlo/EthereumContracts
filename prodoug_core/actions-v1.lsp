...
; INIT
{
	...
	{		
		...
		(when  (= @0x0 "take")
			{
				...
				(if @0xC0
					{ ; In the case of auto-pass.
						...
						;TODO add log functionality here.
		...
		; USAGE: 0 : "_remove", 32 : issue number
		...
		(when (= @0x0 "_remove") ;Remove a poll from the list, and kill the poll contract.
			{
				...
				(when @0x40 ; If we have comments, we include these in the action.
					{					
		...
		(when (= @0x0 "lock")
			{
				...
				[0x40] "getuser"
				[0x60] "somesuperadminpos" ;TODO change
